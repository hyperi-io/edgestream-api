...
File:      edgestream/api/v1/endpoints/log_viewer.py
Language:  Python
...
async def read_last_lines(filepath: str, last_lines: int) -> List[str]:
    ...
    try:
        ...
        Logger.logger.error(f"Read error on {filepath}: {e}")
        return ["[edgestream] error reading log file"]


async def tail_follow(filepath: str) -> AsyncIterator[str]:
    ...
    try:
        ...
        Logger.logger.error(f"Follow error on {filepath}: {e}")
...
async def websocket_endpoint(
    ...
    logfile_raw = urllib.parse.unquote(qp.get("logfile", "")).strip()
    ...
    # Build a lookup map of {normalized_path: trusted_canonical_path}
    try:
        raw_logs = crud.log_viewer.get_all(db=db)
        valid_log_map: Dict[str, str] = {
            os.path.realpath(log.filename): os.path.realpath(log.filename)
            for log in raw_logs
            if log.filename
        }
    except Exception as e:
    ...
    # Check key membership and extract the trusted value from the dict
    user_target = os.path.realpath(logfile_raw)
    trusted_path: Optional[str] = valid_log_map.get(user_target)

    if not trusted_path:
        Logger.logger.warning(f"Unauthorized log access attempt: {logfile_raw}")
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Access Denied")
        return

    if not os.path.exists(trusted_path) or not os.access(trusted_path, os.R_OK):
    ...
    try:
        if lines > 0:
            backlog = await read_last_lines(trusted_path, lines)
            await manager.send_message(websocket, "\n".join(backlog))

        async for line in tail_follow(trusted_path):
            await manager.send_message(websocket, line)

    except WebSocketDisconnect:
        await manager.disconnect(websocket)
    except Exception as e:
        Logger.logger.error(f"WS Streaming Error on {trusted_path}: {e}")
        await manager.disconnect(websocket)
